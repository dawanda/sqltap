...
import com.paulasmuth.sqltap.cache.{CacheGetRequest, CacheStoreRequest}
import com.paulasmuth.sqltap.callbackhell.{ExpirationJob, Worker}
import com.paulasmuth.sqltap.instructions.CTreeInstruction
import com.paulasmuth.sqltap.sqlmapping.{Record, RecordLookupJob}
...
object CTreeCache {
  ...
  def expire(worker: Worker, resource_name: String, record_id: Int) : Unit = {
    if (!sqlmapping.Manifest.has_resource(resource_name))
      throw new ParseException("unknown resource: " + resource_name)

    val ctrees   = CTreeIndex.find(resource_name)
    val resource = sqlmapping.Manifest.resource(resource_name)
